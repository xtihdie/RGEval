...
DEFAULT_TIMING_DIR = PROJECT_ROOT / "data" / "essay" / "results" / "timing"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Summarize total essay test time from timing logs")
...
def main() -> None:
    args = parse_args()
    logs = [Path(args.log)] if args.log else sorted(DEFAULT_TIMING_DIR.glob("essay_test_*.json"))
    if not logs:
