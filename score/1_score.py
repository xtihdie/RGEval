...
from test.prompts import scoring_rubric
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="LLM multi-dimension scoring runner"
    )
...
RUBRIC_ARRAY = []
RUBRIC_ARRAY = load_rubric_dimensions(RUBRIC_ARRAY)
...
def discover_tasks(root: str) -> List[Tuple[int, int]]:
    ...
    for f in os.listdir(root):
        if f.endswith(".csv") and f.split("_")[0].isdigit():
            cid = int(f.split("_")[0])
            for d in range(len(RUBRIC_ARRAY)):
                tasks.append((cid, d))
    return sorted(tasks)
...
def get_origin_level(code: int, score_df: pd.DataFrame) -> str:
    row = score_df[score_df["课例id"] == code]
    if row.empty:
        return ""
    v = row.iloc[0].get("级别", "")
...
def grade_single_dimension(
    ...
    try:
        ...
        rubric = RUBRIC_ARRAY[dim]

        messages = [[
            {"role": "system", "content": scoring_rubric["system"]},
            {
                "role": "user",
                "content": scoring_rubric["user"].format(
                    class_conv=conv,
...
def main():
    ...
    cfg.num_threads = 16 if cfg.agent_name == "zhipu" else 8
